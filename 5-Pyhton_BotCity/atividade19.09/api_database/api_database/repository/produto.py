...
#Serviço: Criar vários produtos
def criar_varios_produtos(produtos):
    try:
        conect = database.criar_db()
        cursor = conect.cursor()
        
        sql = "INSERT INTO produto (descricao, unidade, quantidade, preco_real, preco_dolar) VALUES "
        values_list = []

        for produto in produtos:
            values_list.append(f"('{produto['descricao']}', '{produto['unidade']}', {produto['quantidade']}, {produto['preco_real']}, {produto.get('preco_dolar', 0.0)})")
        
        sql += ', '.join(values_list)
        cursor.execute(sql)
        conect.commit()

        return cursor.lastrowid  # Retorna o ID do último produto inserido, mas se inserir múltiplos pode não ser útil
    except Exception as ex:
        print(f'Erro ao inserir múltiplos produtos: {ex}')
        raise
    finally:
        cursor.close()
        conect.close()
